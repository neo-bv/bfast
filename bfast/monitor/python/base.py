...
class BFASTMonitorPython(BFASTMonitorBase):
    ...
    def fit(self, data, dates, n_chunks=None, nan_value=0):
        ...
        # period = data.shape[0] / np.float64(self.n)
    ...
    def fit_single(self, y):
        ...
        # boundary and breaks
        a = self.mapped_indices[self.n:] / self.mapped_indices[self.n - 1].astype(np.float64)
    ...
    def _create_data_matrix(self, mapped_indices):
        ...
        temp = 2 * np.pi * mapped_indices / np.float64(self.freq)
    ...
    def _log_plus(self, a):
        retval = np.ones(a.shape, dtype=np.float64)
        fl = a > np.e
